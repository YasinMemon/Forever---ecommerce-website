      ...
      <div className="collection my-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {
        products?.map((item) => {
          return <div
           ...
           key={item._id} className="card cursor-pointer w-full p-4 bg-white shadow-lg rounded-md">
            <img 
              className='h-60 w-full object-cover object-center sm:object-top rounded-md'
              src={item.images[0]} 
              alt="Kid Tapered Slim Fit Trouser" 
            />
            <h4 className='font-sans mt-2 text-center font-bold mb-4'>{item.name}</h4>
            {/* <p>{item.description.slice(0,50)}....</p> */}
            <span className="block text-center text-lg font-semibold mt-1">₹{item.price}</span>
          </div>
        })
      }
